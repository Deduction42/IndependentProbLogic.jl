...
Base.:≈(lik1::LogLik, lik2::LogLik) = (lik1.value ≈ lik2.value)
Base.:≈(lik1::Lik, lik2::Lik) = (lik1.value ≈ lik2.value)
...
∧(lik::Lik, liks::Lik...) = mapreduce(nan2one, *, (lik, liks...))
...
"""
∨(lik::AbstractLikelihood, liks::AbstractLikelihood) = ∨(promote(lik,liks...)...)
∨(lik::Lik, liks::Lik...) = !prod(x->nan2one(!x), (lik, liks...))
∨(lik::LogLik, liks::LogLik...) = !sum(x->nan2zero(!x), (lik, liks...))


singleweight(lik::Lik, n) = Lik(lik.value^(1/n))
...
#Logg-ods ratio
logit(lik::Lik) = logit(lik[])
logit(lik::LogLik) = lik[] - (!lik)[]
